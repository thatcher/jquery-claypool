...
(function(){
	...
	var curLocation = (new java.io.File(cwd)).toURL();
	 print("curLocation " + curLocation);
	window.__defineSetter__("location", function(url){
	    var i, scripts, script;
	    var xhr = new XMLHttpRequest();
	    xhr.setRequestHeader("Accept", "text/html");
		xhr.open("GET", curLocation + url);
		xhr.onreadystatechange = function(){
			curLocation = new java.net.URL( curLocation, url );
		    print("<----    Got "+curLocation);
			window.document = xhr.responseXML;
			scripts =  document.getElementsByTagName('script');
            for(i=0;i<scripts.length;i++){
            	script = scripts[i];
            	try{
                	if(script.attributes['rel']=='server'){
                		print('loading script: ' + script.attributes['src']);
                		load(script.attributes['src']);
                		script.setAttribute('rev', script.attributes['src']);
                		script.removeAttribute('src');
                	}else if(script.attributes['rel']=='both'){
                		print('loading script: ' + script.attributes['src']);
                		load(script.attributes['src']);
                	}
            	}catch(e){
	...
	XMLHttpRequest.prototype = {
		...
		setRequestHeader: function(header, value){
			...
			print("Request Header " + header + " : " + value);
		},
			...
			function makeRequest(){
				...
				function handleResponse(){
					...
					// convert bytes to text.
	 	            //Since we are using this on the server-side we need to check to
	 	            //see if the content encoding is gzipped and if so we need to use
	 	            //an appropriate InputStream.
	 	            var contentEncoding = connection.getContentEncoding();
	 	            if( contentEncoding == null || contentEncoding.length == 0 ){
	 	                //logger.debug("No content encoding is specified");
	 	                contentEncoding = "utf-8";
	 	            }
	 	            
					//If its GZIPPED wrap the stream in a gzip decompressor
					var is = (contentEncoding.equalsIgnoreCase("gzip") || 
					...
					// maybe not the most efficient way but faster than a string buffer
					self.responseText = java.nio.charset.Charset.forName("UTF8").
					...
					var accept = self.headers["Accept"]||'text/html';
					var contentType = "";
					if (connection.getContentType() != null) {
						contentType = connection.getContentType().split(";")[0];
					}
					if ( contentType.match(/xml|html/) && accept.match(/xml|html/)) {
						try {
							self.responseXML = new DOMDocument(
								new java.io.ByteArrayInputStream(
									(new java.lang.String(
										self.responseText)).getBytes("UTF8")));
						} catch(e) { /*should do something useful here*/ }
					}
				}
