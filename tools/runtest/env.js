...
(function(){
    ...
    window.logger = Claypool.Logging.getLogger("Claypool.Window");
	// Browser Navigator
	...
	var cwd = this.cwd?this.cwd:"./";
	var curLocationBase = (new java.io.File(cwd)).toURL();
	window.logger.debug("curLocationBase %s",curLocationBase);
	
	window.__defineSetter__("location", function(url){
	    var fileLocation = curLocationBase + url;
	    window.logger.info("Setting Window Location: %s", fileLocation);
	    
		var xhr = new XMLHttpRequest();
		xhr.open("GET", fileLocation, false);
		xhr.onreadystatechange = function(){
		    if(xhr.readyState == 4){
			    curLocation = new java.net.URL(curLocationBase , url);
	            window.logger.debug("Finished Loading %s", curLocation);
    			window.document = new DOMDocument( new java.io.ByteArrayInputStream(
									(new java.lang.String(xhr.responseText)).getBytes("UTF8")
								));
				window.logger.debug("Storing e4x XML object.");
                default xml namespace = "http://www.w3.org/1999/xhtml";
				window.document.xml = new XML(
				    xhr.responseText.replace('<?xml version="1.0" encoding="UTF-8"?>', "")
				);
				window.logger.info("Loading scripts");
                for each(var script in window.document.xml..script){
                    var src = script.@src;
                    if(src.toString().length>0){
                        window.logger.debug("script src : %s", src);
                        load(src);
                    }
                }
	...
	//add 1 so things like if(intervalId) dont fail if timers.length is 0
	var timers = ["root"];
	...
	window.setInterval = function(fn, time){
		...
		print("CREATING NEW THREAD: " + num + " for function: " + fn);
		timers[num] = new java.lang.Thread(java.lang.Runnable({
			run: function(){
				while (true){
				    print("SLEEPING");
